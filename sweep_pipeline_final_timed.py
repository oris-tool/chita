...
import compute_precision_metrics as cpm
...
FAST_EXPERIMENT_GT_CONVERGENCE_THRESHOLD = 1e-4
FAST_EXPERIMENT_MAX_PARAMETER_CASES = 27
...
def build_parameter_combinations(parameter_path):
    ...
    ground_truth_selection = {
        transition_name: dict(parameters[transition_name]["mid"])
        for transition_name in transitions
    }
    for transition_name in transitions:
        ground_truth_selection[transition_name]["level"] = "mid"
    normalized_ground_truth = normalize_stpn_parameter_bundle(ground_truth_selection)
    ground_truth_case = {
        "parameter_bundle": ground_truth_selection,
        "levels": {transition_name: "mid" for transition_name in transitions},
        "case_id": normalized_ground_truth["case_id"],
        "case_index": None,
        "case_run_code": "case_gt",
    }
    ...
    for case_index, combination in enumerate(combinations, start=CASE_INDEX_BASE):
        ...
        if len(all_cases) >= FAST_EXPERIMENT_MAX_PARAMETER_CASES:
            break

    if ground_truth_case is None:
        raise RuntimeError("Could not find the all-mid ground-truth parameter combination.")

    if not any(case["case_id"] == ground_truth_case["case_id"] for case in all_cases):
        if all_cases:
            all_cases[-1] = ground_truth_case
        else:
            all_cases.append(ground_truth_case)
...
def compute_analysis_vs_simulation_metrics_only(
    ...
    output_dir = None
    csv_summaries = None
    analysis_summary, analysis_timings = compute_candidate_summary_fast(
        ground_truth,
        analysis,
        include_moving_avg_metrics=True,
        metric_prefix="analysis",
    )
    baseline_summary, baseline_timings = compute_candidate_summary_fast(
        ground_truth,
        baseline,
        include_moving_avg_metrics=True,
        metric_prefix="simulation",
    )
    if write_csvs:
...
def mean_reciprocal_rank_fast(ground_truth_order, candidate_order):
    if ground_truth_order.shape[0] == 0:
        return 0.0

    reciprocal_rank_total = 0.0
    for timestep in range(ground_truth_order.shape[0]):
        ground_truth_top_subject = ground_truth_order[timestep, 0]
        candidate_positions = sp.np.where(candidate_order[timestep] == ground_truth_top_subject)[0]
        if candidate_positions.size == 0:
            raise ValueError("Ground-truth top subject is missing from candidate ranking.")
        reciprocal_rank_total += 1.0 / (int(candidate_positions[0]) + 1)
    return reciprocal_rank_total / ground_truth_order.shape[0]


def timed_call(metric_timings, metric_name, callback, *args, **kwargs):
    started_at = time.perf_counter()
    result = callback(*args, **kwargs)
    metric_timings[metric_name] = time.perf_counter() - started_at
    return result
...
def compute_candidate_summary_fast(ground_truth, candidate, include_moving_avg_metrics=True, metric_prefix=None):
    summary = {}
    metric_timings = {}
    timing_prefix = f"{metric_prefix}_" if metric_prefix else ""
    tau, p_value_tau = timed_call(
        metric_timings,
        f"{timing_prefix}kendall_tau_seconds",
        sp.ranking_metrics.compute_kendalls_tau_correlation,
        ground_truth,
        candidate,
    )
    spearman, p_value_sp = timed_call(
        metric_timings,
        f"{timing_prefix}spearman_seconds",
        sp.ranking_metrics.compute_spearmans_correlation,
        ground_truth,
        candidate,
    )
    summary["tau"] = tau
    ...
    if not subjects:
        summary["mrr"] = 0.0
        return summary, metric_timings

    max_top_precision = min(sp.MAX_TOP_PRECISION, len(subjects))
    _, ground_truth_matrix = subject_track_matrix(ground_truth)
    _, candidate_matrix = subject_track_matrix(candidate)
    ground_truth_order = ranking_order_rows_descending(ground_truth_matrix)
    candidate_order = ranking_order_rows_descending(candidate_matrix)

    summary["mrr"] = timed_call(
        metric_timings,
        f"{timing_prefix}mrr_seconds",
        mean_reciprocal_rank_fast,
        ground_truth_order,
        candidate_order,
    )
    for top_k in range(1, max_top_precision + 1):
        precision_series = timed_call(
            metric_timings,
            f"{timing_prefix}top_{top_k}_precision_seconds",
            top_precision_series_fast,
            ground_truth_order,
            candidate_order,
            top_k,
        )
        precision_mean = float(sp.np.mean(precision_series)) if precision_series else 0.0
...
def process_and_save_with_timings(
    json_path_p,
    json_path_g,
    *,
    metrics_output,
    ...
    metric_timings = {
        f"{metric_prefix}_brier_score_seconds": 0.0,
        f"{metric_prefix}_ece_seconds": 0.0,
    }
    metrics_dict = {}
    ...
    for subject_id in g_subjects:
        ...
        p_array = sp.np.asarray(p)
        g_array = sp.np.asarray(g)

        started_at = time.perf_counter()
        bs = cpm.calculate_brier_score(p_array, g_array)
        metric_timings[f"{metric_prefix}_brier_score_seconds"] += time.perf_counter() - started_at

        started_at = time.perf_counter()
        ece = cpm.calculate_ece(p_array, g_array, M=M)
        metric_timings[f"{metric_prefix}_ece_seconds"] += time.perf_counter() - started_at
...
def summarize_metric_timing_averages(comparison_summaries):
    totals = {}
    counts = {}
    for summary in comparison_summaries:
        for metric_name, elapsed_seconds in summary.get("metric_timing_seconds", {}).items():
            if elapsed_seconds is None:
                continue
            totals[metric_name] = totals.get(metric_name, 0.0) + float(elapsed_seconds)
            counts[metric_name] = counts.get(metric_name, 0) + 1

    averages = {
        metric_name: totals[metric_name] / counts[metric_name]
        for metric_name in totals
        if counts.get(metric_name)
    }
    return dict(sorted(averages.items()))


def print_metric_timing_averages(metric_timing_averages):
    print("\nAverage metric computation times (seconds):")
    if not metric_timing_averages:
        print("  No metric timings were collected.")
        return
    for metric_name, average_seconds in metric_timing_averages.items():
        print(f"  {metric_name}: {average_seconds:.6f}")
...
def run_comparison_task(
    ...
    if LOG_PRECISION_METRICS:
        ...
        with contextlib.redirect_stdout(metrics_stdout):
            prediction_metrics_summary, prediction_metric_timings = process_and_save_with_timings(
                analysis_path,
                ground_truth_path,
                M=10,
            ...
            baseline_metrics_summary, baseline_metric_timings = process_and_save_with_timings(
                baseline_path,
                ground_truth_path,
                M=10,
    ...
    summary = {
        ...
        "metric_timing_seconds": {
            **comparison_metrics.get("metric_timing_seconds", {}),
            **prediction_metric_timings,
            **baseline_metric_timings,
        },
...
def main(argv=None):
    ...
    for index, dataset_path in enumerate(progress(dataset_paths, desc="Running GT simulations"), start=1):
        ...
        if gt_results is None:
            ...
            gt_results = run_dataset_simulations(
                ...
                iterations_cap=100_000,
                convergence_threshold=FAST_EXPERIMENT_GT_CONVERGENCE_THRESHOLD,
                fine_grained=False,
                time_step_hours=TIME_STEP,
                seed=30,
                prune_after_positive_test=False,
    ...
    output_paths = persist_comparison_outputs(save_path, quartile_label, comparison_summaries)
    metric_timing_averages = summarize_metric_timing_averages(comparison_summaries)
    write_json(os.path.join(save_path, "metric_timing_averages.json"), metric_timing_averages)
    update_run_completion_metadata(
        save_path=save_path,
        quartile_label=quartile_label,
        comparison_summaries=comparison_summaries,
    ...
    print_metric_timing_averages(metric_timing_averages)
